...
# соберём sslocal под все ABI и переименуем в ss-local
...
echo "=== tun2socks (eycorsican/go-tun2socks) ==="
git clone --depth=1 https://github.com/eycorsican/go-tun2socks.git t2s
pushd t2s >/dev/null
# Для отладки, если вдруг снова не найдёт пакет:
ls -la
ls -la cmd || true

CGO_ENABLED=1 CC="$ANDROID_NDK_HOME/toolchains/llvm/prebuilt/linux-x86_64/bin/aarch64-linux-android24-clang" GOOS=android GOARCH=arm64 \
  go build -trimpath -ldflags="-s -w" -o "$OUT/arm64-v8a/tun2socks" ./cmd/tun2socks

CGO_ENABLED=1 CC="$ANDROID_NDK_HOME/toolchains/llvm/prebuilt/linux-x86_64/bin/armv7a-linux-androideabi24-clang" GOOS=android GOARCH=arm GOARM=7 \
  go build -trimpath -ldflags="-s -w" -o "$OUT/armeabi-v7a/tun2socks" ./cmd/tun2socks

CGO_ENABLED=1 CC="$ANDROID_NDK_HOME/toolchains/llvm/prebuilt/linux-x86_64/bin/i686-linux-android24-clang" GOOS=android GOARCH=386 \
  go build -trimpath -ldflags="-s -w" -o "$OUT/x86/tun2socks" ./cmd/tun2socks

CGO_ENABLED=1 CC="$ANDROID_NDK_HOME/toolchains/llvm/prebuilt/linux-x86_64/bin/x86_64-linux-android24-clang" GOOS=android GOARCH=amd64 \
